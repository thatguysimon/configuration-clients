...
export default class OSVars {
  ...
  private constructor() {
    ...
    this.__canExit = (
      !process.env[PRINT_CONFIG_USAGE_ENV_VAR] &&
      !process.env[DUMP_CONFIG_ENV_VAR])
   ...
   */
  public static register(varKey: string, varDescription: string, varType: OSVarType = OSVarType.String, defaultValue: any = undefined): void {
    return OSVars.instance.__register(
      varKey, varDescription, varType, false, defaultValue
    )
  }
  ...
  * @param varType - type of var (for validation) default is "string" can also be "int", "float", "bool"
  */
  public static registerMandatory(varKey: string, varDescription: string, varType: OSVarType = OSVarType.String): void {
    return OSVars.instance.__register(
      varKey, varDescription, varType, true
    )
  }
  ...
  */
      ...
      if (!(varKey in process.env)) {
        if (varObj.isMandatory) {
          this.__criticalFault(
            `Missing mandatory os env var ${varKey} (${varObj.description})`
          )
        }
      } else {
   ...
   */
  private __register(varKey: string, varDescription: string, varType: OSVarType = OSVarType.String, isMandatory: boolean = false, defaultValue?: any) {
    // sanity checks...
    ...
    if (defaultValue !== undefined && isMandatory) {
      this.__criticalFault(
        `defining var ${varKey} as mandatory with default value doesn't make sense!`
      )
    }
